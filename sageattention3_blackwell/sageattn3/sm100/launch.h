...
///////////////////////////////////////////////////////////////////////////////
// Run SM100 Flash Attention Forward
///////////////////////////////////////////////////////////////////////////////
...
void run_flash_fwd_sm100(Flash_fwd_params& params, cudaStream_t stream) {
    ...
    using CollectiveMainloop = CollectiveMainloopFwdSm100<Kernel_traits, Is_causal>;
    using CollectiveEpilogue = CollectiveEpilogueFwdSm100<Kernel_traits>;
    using Scheduler = StaticPersistentTileScheduler;

    // Convert host arguments to device parameters
    typename CollectiveMainloop::Params mainloop_params =
        CollectiveMainloop::to_underlying_arguments({
            ...
            {params.unpadded_seqlen_k, params.d, params.h_k, params.b},  // unpadded_shape_K
            static_cast<Element const*>(params.v_ptr),
            {params.d, params.seqlen_k, params.h_k, params.b},  // shape_V (transposed)
            ...
            {params.d, params.seqlen_k, params.h_k, params.b},  // shape_SFV
    ...
    // Get kernel pointer
    void* kernel = (void*)compute_attn_ws_sm100<Kernel_traits, Is_causal, Scheduler>;
    ...
    // SM100 supports flexible cluster shapes (unlike SM120 which is fixed to 1x1x1)
    dim3 grid_dims = Scheduler::get_grid_dim(scheduler_args, 170);  // Target 170 CTAs per SM
...
///////////////////////////////////////////////////////////////////////////////
// Template dispatch for head dimension
///////////////////////////////////////////////////////////////////////////////
    ...
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        BOOL_SWITCH(params.per_block_mean, per_block, [&] {
            if constexpr (Headdim == 64) {
                run_flash_fwd_sm100<
                    Flash_fwd_kernel_traits_sm100<64, 128, 128, 3, 1, per_block, T, O>,
                    Is_causal
                >(params, stream);
            } else if constexpr (Headdim == 128) {
                run_flash_fwd_sm100<
                    Flash_fwd_kernel_traits_sm100<128, 128, 128, 3, 1, per_block, T, O>,
                    Is_causal
                >(params, stream);
            } else {
