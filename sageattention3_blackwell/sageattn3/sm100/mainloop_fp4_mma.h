...
>
struct Flash_fwd_kernel_traits_sm100_fp4_mma {
    // Configuration
...
struct CollectiveMainloopFwdSm100FP4Mma {
    ...
    using Mask = std::conditional_t<Is_causal, FP4MmaCausalMask, FP4MmaNoMask>;
    ...
    CUTLASS_DEVICE static void accumulate_pv(
        ...
        for (int sf_block = 0; sf_block < NumSFPerHead; ++sf_block) {
            ...
            for (int dd = 0; dd < SFVectorSize; dd += 2) {
                ...
                thread_output[dim_idx] += scaled_weight * decode_fp4(v_byte, 0);
    ...
    CUTLASS_DEVICE void operator()(
        ...
        for (int n_tile = 0; n_tile < num_kv_tiles; ++n_tile) {
            ...
            // Compute QK^T for this tile (scalar fallback)
            // Store scores temporarily and compute softmax
            float tile_scores[kBlockN];
            ...
            // Now load V tile (reuses K SMEM space) and accumulate PV
...
struct Sm100FlashFwdKernelFP4Mma {
    ...
    using CollectiveMainloop = CollectiveMainloopFwdSm100FP4Mma<Ktraits, Is_causal>;
    ...
    // Reuse Arguments structure from base FP4 kernel
    struct Arguments {
        int seqlen_q;
        int seqlen_k;
        int head_dim;
