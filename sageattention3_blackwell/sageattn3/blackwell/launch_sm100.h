...
#include "kernel_ws_sm100.h"
#include "kernel_traits_sm100.h"
...
void run_flash_fwd_sm100(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    using CollectiveMainloop = flash::CollectiveMainloopFwdSm100<Kernel_traits, Is_causal>;
    ...
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        BOOL_SWITCH(params.per_block_mean, per_block, [&] {
            if constexpr (Headdim == 64 || Headdim == 128) {
                run_flash_fwd_sm100<
                    Flash_fwd_kernel_traits_sm100<Headdim, 128, 128, 3, 1, per_block, T, O>,
                    Is_causal
                >(params, stream);
            } else {
